...
echo 'Repository("selwtf","https://storage.googleapis.com/sel-wtf-turris/omnia/stable/packages", {verification="sig", ocsp=false, pubkey="file:///etc/updater/keys/selwtf.release.key"})' > /etc/updater/conf.d/selwtf.lua

echo "Adding selwtf to opkg package feeds"
cp /etc/updater/keys/sel.release.pub /etc/opkg/keys/0ccc6cfbacaa5e8b
cp /etc/updater/keys/sel.standby.pub /etc/opkg/keys/57e5e1f8717bb7fc
echo "src/gz selwtf https://storage.googleapis.com/sel-wtf-turris/omnia/stable/packages" >> /etc/opkg/customfeeds.conf
