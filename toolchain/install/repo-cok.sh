...
echo 'Repository("selwtf","https://storage.googleapis.com/sel-wtf-turris/omnia/cok/stable/packages", {verification=sig, ocsp=false, priority=60, pubkey="file:///etc/updater/keys/selwtf.release.pub"})' > /etc/updater/conf.d/selwtf-repo.lua

echo "Adding kernel change instruction to Turris updater"
echo 'Install("kernel", { repository = {"selwtf"} })' > /etc/updater/conf.d/selwtf-cok.lua